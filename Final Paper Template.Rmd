...
ggplot(data.frame(x = c(0, 1)), aes(x = x)) +
  ...
  annotate("text",
           ...
           color = "red")
```
When we performed a likelihood ratio test on our maximum likelihood estimate, we got a value$$W = 121.$$
...
Figure 1 compares the initial priors for the Pfizer analysis (Beta$(0.700102,1)$) and our two alternative priors (Beta$(43.04,43.04)$ and Beta$(85.63,85.63)$). As shown, the Pfizer prior places more mass toward large values of $\psi$, whereas the alternative priors are more tightly concentrated around $\psi=0.25$. After observing $t=8$ infections out of $n=170$ total participants in the vaccine group (see data in table 1), these priors update to yield the posterior distributions shown in Figure 1. 
