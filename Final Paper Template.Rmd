...
ggplot(data.frame(x = c(0, 1)), aes(x = x)) +
  ...
  labs(x = expression(psi),
       y = expression(L(psi)),
       title = "Likelihood Function Plotted with Maximizing Value Marked and Labeled")+
...
The p-value associated with this value of $W$ is $\sim0$.
...
Because our observed likelihood ratio p-value is so astronomically low, we might be interested in knowing what would be the highest value we could take to be our null assumption for $\psi$, and still get a p-value lower than 0.05. We can create a plot that visualizes this:
