...
plot(ml.binom) +
  labs(title = "Second-Degree Taylor Approximation of Log-likelihood Function",
       ...
       y = "Log-Likelihood")
```
To find the estimate of $\psi$ that maximizes this likelihood function, we took the first derivative and maximized. The whole derivation, from likelihood function to final estimate formula, can be found in the Appendix. Our final formula is given by:

$$\hat{\psi}_0^{mle} = \frac{n-2X}{n-X}$$
Given our estimate, we now perform a likelihood ratio test to test our hypotheses. We can initially find the likelihood ratio, $\Lambda$. It has formula given by:
...
The frequentist maximum likelihood estimation yields an estimate of 0.9506, which is significantly higher than the value we are testing against, 0.3. In testing the statistical significance of this result, the likelihood ratio test obtains the value $W = 121.6012$. Considering under the null hypothesis, $W\sim\chi_1^2$, which has expected value 1 and variance 2, this value is extraordinarily high, and it is thus no surprise that we get a p-value that approximates 0. We thus reject the null hypothesis that the true value of our parameter $\psi$ is equal to $0.3$. This is right in line with the extremity of our estimate and the magnitude of the difference between it and the baseline we test against. It is also in agreement with the results achieved by the large sample confidence interval, as well as the results yielded by the bayesian analysis. In fact, we could declare any value less than 0.906 for minimum efficacy and our data would still constitute statistically significant evidence at the significance level of $\alpha = 0.05$ that the vaccine has a high enough efficacy (See Appendix for plot).
...
Both frequentist and Bayesian frameworks affirmed BNT162b2's high efficacy, with point estimates exceeding 94.8\% and overwhelming statistical evidence against the null hypothesis ($\psi \leq 30\%$). Frequentist $p$-values ($<0.0001$) and Bayesian posterior probabilities ($>99.99\%$) aligned in rejecting $H_0$, corroborating Pfizer’s original findings. 

Strengths of the Bayesian approach included explicit usage of prior belief and probabilistic efficacy interpretations, whereas frequentist methods provided precise, assumption-driven estimates. However, Bayesian results exhibited mild sensitivity to prior choice, and frequentist intervals relied on large-sample approximations. Despite these trade-offs, the similar conclusion from the results across frameworks reinforced confidence in BNT162b2’s efficacy. 
